...
def SubCategoryView(request,catId):
    ...
    for i in subCategoryData:
        x=ProductModel.objects.get(subCategory=i.pk)
        print("hiiiippp", x)
        a={
            'name':x.name,
            'image':x.image,
        ...
        data.append(a)
    return render(request,'subcategory.html',{'subCatData':data,'navData':sessionData})

def userproductdetailview(request,id):
    sessionData = sessionDataView(request)
    productData = ProductModel.objects.get(id=id)
   
    orderProductDataNew = CartModel.objects.filter(productId=id)
    iOrderCountNew = 0
        ...
        if request.session.has_key('sessionId'):
            ...
            if productData.stock > iOrderCount:
                ...
                if len(cartDataCheck) > 0 :
                    ...
                    cartModel.orderId = 0
                    cartModel.checkin_date = checkin_date
                    cartModel.checkout_date = checkout_date
                    cartModel.checkin_time = checkin_time
                    cartModel.checkout_time = checkout_time
                    cartModel.guests = guests
...
def user_productdetailview(request,id):
    sessionData = sessionDataView(request)
    productData = ProductModel.objects.get(id=id)
...
def shopview(request,subCatId):
    if subCatId==0:
        ...
        a=CategoryModel.objects.filter(id=subCatId)
        productData = ProductModel.objects.filter(subCategory=a.pk)
...
""" def userproductdetailview(request,productId):
    sessionData = sessionDataView(request)
    productDetailData = ProductModel.objects.get(pk=productId)
    return render(request,'user-product-detail.html',{'productDetailData':productDetailData,'navData':sessionData}) """
...
def direct_buy(request):    
    ...
    if request.session.has_key('sessionId'):  
        if request.method=="POST":
            ...
            model=CartModel()
            model.orderId="0"
            model.userId=request.session['sessionId']
            model.productId=id
            model.qty="1"
            model.productPrice=product.price
            model.totalPrice=str(int(model.qty)*product.price)
            model.save()
            # abc=CartModel.objects.get(productId=id)      
            request.session['shippingUserId'] = request.session['sessionId']
            request.session['shippingfname']=request.session['sessionFirstName']
            request.session['shippinglname']=request.session['sessionLastName']
            request.session['shippingemail']=request.session['sessionEmail']
            request.session['shippingcontact']=request.session['sessionContact']
            request.session['shippingaddress']=request.session['sessionAddress']
            request.session['shippingcity']=""
         
            request.session['shippingsubTotal']=product.price
            request.session['shippinggstPercentage']=1
            request.session['shippinggstAmount']=0
            request.session['shippingdeliveryCharge']=0
            request.session['shippinggrantTotal']=product.price
...
def ordersuccessview(request):
    sessionData = sessionDataView(request)
    data=CategoryModel.objects.all()[2:4]
    return render(request,'ordersuccess.html',{'navData':sessionData,'data':data})
...
def ratingView(request,productId):
    ...
    if request.method=='POST':
        ...
        rateModel.save() 
        return render(request,'rating.html',{'navData':sessionData,"productData":productData,'ratingsend':'Thanks For Rating '})
